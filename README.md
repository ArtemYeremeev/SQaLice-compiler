...
Функция *Search* обеспечивает компиляцию запроса с LIKE-поиском по переданной строке и параметру. Основные параметры компиляции запроса (выборка полей, условия, ограничения)
работают аналогично основному запросу *Get*. Функция поиска дополнительно принимает 2 параметра для поиска - поле, по которому осуществляется поиск (__searchField__) и строки запроса
(__searchQuery__). Поиск протестирован для работы с полями строкового и цифрового формата (varchar, text, integer, numeric). Корректность работы поиска по другим полям не гарантирована.

## Примеры поисковых запросов

Запрос с пустыми основными параметрами и текстовым полем __title__

```http
http://url/.../query=??&searchQuery=some
```
...
Запрос со сложными основными параметрами и цифровым полем __ID__

```http
http://url/.../query=title?ID!=1?ID,desc,10,0&searchQuery=12
```

```sql
select q.title from v_test q where q.id::text like '%%1%%' and q.id != 1 order by q.id desc limit 10 offset 0
```
